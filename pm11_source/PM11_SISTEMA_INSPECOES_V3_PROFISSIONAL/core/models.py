...
def list_plans(pid,search='',status='',cycle_code=''):
    c=get_conn()
    try:
        ...
        return rows(c.execute(f"SELECT * FROM inspection_plans WHERE {' AND '.join(where)} ORDER BY code",args).fetchall())
    finally:c.close()

def get_plan(plan_id):
    c=get_conn();
    try:return row(c.execute('SELECT * FROM inspection_plans WHERE id=?',(plan_id,)).fetchone())
    finally:c.close()
...
def create_plan(pid,data):
    ...
    c=get_conn()
    try:
        cur=c.execute('''INSERT INTO inspection_plans(project_id,code,description,char_count,center_code,process_code,type_code,line_code,subarea_code,suffix,cycle_code,cycle_value,unit,text_cycle,horizon,counter,status)
                         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(pid,d['code'],d['description'],d['char_count'],d['center_code'],d['process_code'],d['type_code'],d['line_code'],d['subarea_code'],d['suffix'],d.get('cycle_code'),d.get('cycle_value'),d.get('unit'),d.get('text_cycle'),d.get('horizon'),d.get('counter',''),d.get('status','ACTIVE')))
...
def update_plan(plan_id,data):
    ...
    c=get_conn()
    try:
        c.execute('''UPDATE inspection_plans SET code=?,description=?,char_count=?,center_code=?,process_code=?,type_code=?,line_code=?,subarea_code=?,suffix=?,cycle_code=?,cycle_value=?,unit=?,text_cycle=?,horizon=?,counter=?,status=?,updated_at=? WHERE id=?''',
        (d['code'],d['description'],d['char_count'],d['center_code'],d['process_code'],d['type_code'],d['line_code'],d['subarea_code'],d['suffix'],d.get('cycle_code'),d.get('cycle_value'),d.get('unit'),d.get('text_cycle'),d.get('horizon'),d.get('counter',''),d.get('status','ACTIVE'),_now(),plan_id));c.commit();return get_plan(plan_id)
...
def bulk_update_plans(ids,updates):
    allowed={'cycle_code','status','counter'}; upd={k:v for k,v in updates.items() if k in allowed}
    if not upd:return
...
def dashboard(pid):
    ...
    try:
        ...
        return {'plans':plan_count,'items':item_count,'characteristics':char_count,'routes':routes,'total_minutes':float(total_min or 0),'issues':{'missing_time':missing_time,'missing_route':missing_route,'missing_characteristics':missing_chars},'by_cycle':by_cycle,'by_type':by_type,'top_methods':top_methods}
    finally:c.close()
...
def duplicate_project(pid,new_name=None):
    ...
    try:
        ...
        for p in c.execute('SELECT * FROM inspection_plans WHERE project_id=? ORDER BY id',(pid,)).fetchall():
            cols=['code','description','char_count','center_code','process_code','type_code','line_code','subarea_code','suffix','cycle_code','cycle_value','unit','text_cycle','horizon','counter','status','row_color']
...
def bulk_update_plans(ids,updates):
    allowed={'cycle_code','status','counter','row_color'};upd={k:v for k,v in updates.items() if k in allowed}
