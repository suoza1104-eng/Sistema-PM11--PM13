...
FIELDS={
 'plans':{
  ...
  'cycle_value':['ciclo'], 'unit':['unid.','unid','unidade'], 'text_cycle':['texto ciclo','texto do ciclo'], 'horizon':['horiz abertura','horizonte de abertura'], 'offset_days':['offset','offset dias']},
 'items':{
...
def confirm_import(project_id,path,mode='MERGE',mapping_override=None):
    ...
    try:
        ...
        for raw in d['plans']:
            p=_infer_plan_cycle(dict(raw),cat);code=(p.get('code') or '').strip().upper();desc=(p.get('description') or '').strip();cv=_int(p.get('cycle_value'),None);unit=(p.get('unit') or '').upper();txt=p.get('text_cycle') or '';h=_num(p.get('horizon'),None);offset=_int(p.get('offset_days'),None) or parse_offset_from_text(desc,code)
            ...
            if ex:
                pid=ex[0];c.execute('UPDATE inspection_plans SET description=?,char_count=?,cycle_code=?,cycle_value=?,unit=?,text_cycle=?,horizon=?,offset_days=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',(desc,len(desc),cycle_code,cv,unit,txt,h,offset,pid));stats['plans_updated']+=1
            else:
        ...
        try:
            ...
            validate_pm11_project(project_id)
        except Exception:pass
        return {'stats':stats,'preview':{k:v for k,v in preview.items() if k!='data'}}
...
def export_project(project_id,balance_rows=None):
    ...
    pr=[['Plano','Descrição do Plano','Qtd caracter','Ciclo','Unid.','Texto Ciclo','Horiz Abertura','Offset']]+[[p['code'],p['description'],p['char_count'],p['cycle_value'],p['unit'],p['text_cycle'],p['horizon'],p.get('offset_days')] for p in plans]
