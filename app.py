    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == '/api/export/systems':
                ...
                content = export_service.export_pm13_systems_xlsx(proj_id, raw_item_ids)

                safe_name = re.sub(r'[^A-Za-z0-9_-]+', '_', project.get('name') or 'PM13')
                stamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
                filename = f"CARGA_SISTEMAS_PM13_SELECIONADOS_{stamp}.xlsx" if raw_item_ids else f"CARGA_SISTEMAS_PM13_{safe_name}_{stamp}.xlsx"
                self.send_response(200)
                self.send_header('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
                self.send_header('Content-Disposition', f'attachment; filename="{filename}"')
